...
> **Early, real code** — **M0 through M6 have shipped (v0.6.7)**: the core backup engine,
...
**M0 through M6 have shipped (v0.6.7)** — the core engine, cluster disaster recovery, restore,
...
**0.6.7 is offered for testing in real conditions, not for production.** The difference is
...
| 0.6.7 — Nine days of a report condemning volumes it was never allowed to look at | [crucible-m6.7](https://crystalbackup.github.io/CrystalBackup/reports/crucible-m6-7.html) — the same **93 checks** once more, 0 failed and 0 skipped, in 2h40m25s on a freshly provisioned six-node cluster; the count is not what is distinctive about this round but the **nine campaigns** it took, six of them red and **not one of the reds the product**: two harness checks that had been winning a coin flip since 0.6.5, two cold-start costs, one campaign disqualified by an operator error, and one crash-path leak invariant that could never observe the reaper it depended on |
...
The Crystal Backup column is **v0.6.7 as shipped** — every ✅ below is code you can install
...
| Capability | Crystal Backup *(v0.6.7)* | Velero | K8up | VolSync | Kasten K10 |
|---|:--:|:--:|:--:|:--:|:--:|
...
| **M6** | Observability hardening & production readiness — metrics catalogue, dashboards, alert rules, traces, restore-fidelity gate | shipped — **v0.6.3**, current |
