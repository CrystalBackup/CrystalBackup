...
| `namespacesFailed` _integer_ | namespacesFailed at least one PVC. |  |  |
...
| `snapshotsForgotten` _integer_ | snapshotsForgotten count. |  |  |
...
| `restoredVolumes` _integer_ | restoredVolumes count. |  |  |
...
| `forcePathStyle` _boolean_ | forcePathStyle selects path-style addressing (required by most non-AWS gateways). |  |  |
