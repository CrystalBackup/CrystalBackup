...
`crystalbackup_backup_failures` and `crystalbackup_backup_failures_total` are not two spellings of one number. The gauge counts failed Backup objects that still EXIST, so it falls back to zero when a schedule's history limit deletes them; the counter records every Backup that ever reached a failed phase, and only a restart resets it. Alert on the counter with `increase()`; put the gauge on a dashboard when the question is what wreckage is lying around right now.
