...
(`.github/workflows/images.yml`, owned by the supply-chain/release track). You rarely build
images locally, but you can **verify** a published one (cosign on `PATH`):
...
cosign verify-attestation "$IMG" --type slsaprovenance \
  --certificate-identity-regexp '^https://github.com/CrystalBackup/CrystalBackup/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
cosign verify-attestation "$IMG" --type openvex \
  --certificate-identity-regexp '^https://github.com/CrystalBackup/CrystalBackup/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
...
The point of the VEX is to answer, for an image that already shipped, *which* of today's
findings actually reach running code. Extract the most recent attested document and feed it to
your scanner:
...
cosign verify-attestation "$IMG" --type openvex \
  --certificate-identity-regexp '^https://github.com/CrystalBackup/CrystalBackup/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  | jq -r '.payload' | base64 -d | jq '.predicate' > crystalbackup.openvex.json

trivy image "$IMG" --vex crystalbackup.openvex.json --severity CRITICAL,HIGH
```

Two things worth knowing before you rely on it:

- **A scanner will not tell you if the document failed to apply.** Trivy accepts a VEX file
  that matches nothing and reports every finding anyway, without an error. If the count does
  not change, compare the `products[]."@id"` in the document against the purl your scanner
  computes for the image (`trivy image "$IMG" --format cyclonedx | jq -r
  '.metadata.component.purl'`) — a mismatch is silent.
- **Only the three most recent releases keep a refreshed document** (adr/0012 support window).
  `.github/workflows/vex-refresh.yml` re-runs the analysis daily against the source that
  produced each supported digest and re-attests, without rebuilding. Older digests stay signed
  and SBOM'd, but their VEX is a point-in-time statement.

Every statement is generated by `govulncheck` in source mode (symbol-level reachability) and
never written or edited by hand — `build/vex/generate-vex.sh` is the whole of it, and it is
runnable locally.
...
| Images / release | [`../.github/workflows/images.yml`](../.github/workflows/images.yml) | tag `v*`, PR (paths) | apko multi-arch build, CVE gate, cosign sign, SBOM, **OpenVEX**, SLSA provenance, GHCR publish |
| VEX refresh | [`../.github/workflows/vex-refresh.yml`](../.github/workflows/vex-refresh.yml) | daily cron, manual | re-run reachability against supported published digests and re-attest — **no rebuild** |
