...
|---|---|
| `slides/slides.md` | Pill de couverture (`v0.6.2 · M0–M6 livrés`), disclaimer `whoami`, note du tableau comparatif (« v0.6.2 livrée »), slide « Ce que vous n'avez PAS vu » (M7/M8/M9 → recaler quand un milestone sort **ou quand la roadmap est re-arbitrée**, cf. 2026-08-02), `--version 0.6.2` du `helm install` (CTA), le compte « 82 checks » et « 20 ADRs ». **Chercher aussi les mentions de milestone dans le CORPS des slides** : la re-arbitration a recalé les panneaux « PAS vu » mais laissé « Object Lock, M8 » dans la slide effacement, alors que l'immutabilité est passée en M9 — un `grep -n 'M[789]'` sur tout le fichier, pas seulement sur la slide de fin |
| `conferencehall.md` | Mentions de version, « M0–M6 », « 82 checks » dans l'abstract et la description (au 0.6.2 : aucun numéro de version n'y figure, seul « 82 checks » est daté) |
| `demo/00-prep.sh` | Le commentaire « releases ≤ 0.6.1 » sur l'extraction de la KEK — **0.6.2 embarque le fix `age.ParseIdentities`**, donc le `grep` y est désormais facultatif et le commentaire le dit ; `CHART_VERSION` (vide = dernière release publiée) si on veut épingler |
