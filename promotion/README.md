...
|---|---|
| `slides/slides.md` | Pill de couverture (`v0.6.3 · M0–M6 livrés`), disclaimer `whoami`, note du tableau comparatif (« v0.6.3 livrée »), slide « Ce que vous n'avez PAS vu » (M7/M8/M9 → recaler quand un milestone sort **ou quand la roadmap est re-arbitrée**, cf. 2026-08-02), le bloc `kubectl create namespace` + labels PSA + `--version 0.6.3` du `helm install` (CTA — **le miroir de `website/…/start/install.md`, y compris l'absence de `--create-namespace` : `namespace.create` vaut `false` par défaut depuis 0.6.3**), le compte « 90 checks », le nom du rapport lié sur la slide crucible (`crucible-m6-3.html`) et « 20 ADRs » (compter avec `ls spec/adr/`, ne pas croire la slide). **Chercher aussi les mentions de milestone dans le CORPS des slides** : la re-arbitration a recalé les panneaux « PAS vu » mais laissé « Object Lock, M8 » dans la slide effacement, alors que l'immutabilité est passée en M9 — un `grep -n 'M[789]'` sur tout le fichier, pas seulement sur la slide de fin |
| `conferencehall.md` | Mentions de version, « M0–M6 », « 90 checks » dans l'abstract et la description (au 0.6.3 : aucun numéro de version n'y figure, seul « 90 checks » est daté) |
