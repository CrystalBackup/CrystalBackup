...
|---|---|
| `slides/slides.md` | Pill de couverture (`v0.6.7 · M0–M6 livrés`), disclaimer `whoami`, note du tableau comparatif (« v0.6.7 livrée »), slide « Ce que vous n'avez PAS vu » (M7/M8/M9 → recaler quand un milestone sort **ou quand la roadmap est re-arbitrée**, cf. 2026-08-02), le bloc `kubectl create namespace` + labels PSA + `--version 0.6.7` du `helm install` (CTA — **le miroir de `website/…/start/install.md`, y compris l'absence de `--create-namespace` : `namespace.create` vaut `false` par défaut depuis 0.6.3** ; si le site est en retard d'une release, c'est le site qui a tort — la version qui fait foi est celle de `charts/crystal-backup/Chart.yaml`), le compte « 93 checks » **et la durée** sur la slide crucible, le nom du rapport lié (`crucible-m6-7.html`) et « 20 ADRs » (compter avec `ls spec/adr/`, ne pas croire la slide). **La note sous le rapport raconte aussi le nombre de campagnes et pourquoi les rouges étaient rouges** — c'est un fait de release, à ré-écrire à chaque fois (au 0.6.7 : neuf campagnes, six rouges, aucun rouge imputable au produit). **Chercher aussi les mentions de milestone dans le CORPS des slides** : la re-arbitration a recalé les panneaux « PAS vu » mais laissé « Object Lock, M8 » dans la slide effacement, alors que l'immutabilité est passée en M9 — un `grep -n 'M[789]'` sur tout le fichier, pas seulement sur la slide de fin. **Et l'état des critères de sortie de M6** : les notes orateur des slides coexistence et « PAS vu » disent où en sont le soak, le pilote et la revue PodSecurity — au 0.6.7 le soak **tourne toujours** (15 jours, sur un vrai cluster de build, démarré en **0.6.5** et volontairement laissé en 0.6.5 jusqu'à la fin de la fenêtre), le pilote n'a pas commencé et la revue PodSecurity écrite n'existe pas ; ne jamais décrire le soak comme fait avant que `hack/soak/collect.sh` ait rendu son archive. **Attention au numéro de release qui a lancé le soak** : au 0.6.6 les deux notes se contredisaient (0.6.5 dans l'une, 0.6.6 dans l'autre) — c'est la release de **démarrage** qui est le fait, elle ne bouge pas d'une release à l'autre |
| `conferencehall.md` | Mentions de version, « M0–M6 », « 93 checks » dans l'abstract et la description (au 0.6.7 : aucun numéro de version n'y figure, seul « 93 checks » est daté — inchangé depuis 0.6.5) |
| `demo/00-prep.sh` | Le commentaire « releases ≤ 0.6.1 » sur l'extraction de la KEK — **le fix `age.ParseIdentities` est embarqué à partir de 0.6.2**, donc le `grep` y est facultatif sur la release installée et le commentaire le dit (y recaler le numéro de release courant : 0.6.7) ; `CHART_VERSION` (vide = dernière release publiée) si on veut épingler |
